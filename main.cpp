...
void
getExtreamalRegions(const val_t *image,std::vector<idx_t>& er )
{
	...
	*/
	{
		...
		std::ofstream otf("result.txt");
		std::ofstream posAndIntensity("posAndIntensity.txt");
		...
		for (int i=0;i<extreamSize;i++)
		{
			...
			for(int j=0;j<tmpSize;j++){
				...
				posAndIntensity<<tmpRow<<" "<<tmpCol<<" "<<currentIntensity<<std::endl;
				totalIntensity=totalIntensity+currentIntensity;
				colIntensity=colIntensity+p.col*currentIntensity;
				rowIntensity=colIntensity+p.row*currentIntensity;
			}

			Point center;
			center.col= colIntensity/totalIntensity;
			center.row=rowIntensity/totalIntensity;
			centers.push_back(center);
			otf<<center.row<<" "<<center.col<<std::endl;
			//posAndIntensity<<"*****************************"<<std::endl;
...
done_all_neighbors:;
	...
	* -------------------------------------------------------------- */
	...
	for(i = 0 ; i < ner ; ++i) {
		...
		/*当前er对应的像素树里面的节点，一个er代表一个像素树里面的子树，er[i].index表示该子树的根*/
...
remove :
...
#ifdef SIXTEEN
	std::ofstream outf2("16-max.txt");
	for (int i=0;i<ner;i++)
	{
		if (er[i].max_stable)
		{
			outf2<<"ner: "<<i<<" value"<<(int)er[i].value<<" area"<<(int)er[i].area<<" parent"<<(int)er[i].parent<<std::endl;
	...
	* -------------------------------------------------------------- */
	...
	for (i = 0 ; i < ner ; ++i) {
		...
		if ((er [i] .max_stable)&&(isOneBranch(er,i))){ mer [j++] = er [i] .index ;
		exIdx.push_back(er [i] .index+1);}
	}

	getExtreamalRegions(im,exIdx);
